...
#Leer los ya datos tratados
#8 características, 4 medias y 4 medianas
...
#Convertirlos con Numpy los datos leidos
dataMean = datasetCSVMean.to_numpy()
dataMedian = datasetCSVMedian.to_numpy()
data = datasetCSV.to_numpy()

#El target (objetivo) es el mismo para los tres (Los usuarios)
target = []
#Las features (características) seran distintas
dataProcMean = []
dataProcMedian = []
dataProc = []
...
X_trainMean, X_testMean, y_trainMean, y_testMean = train_test_split(
             XMean, y, test_size = 0.2, random_state=7) #7
...
clfMean = DecisionTreeClassifier(random_state=7)
clfMean = clfMean.fit(X_trainMean,y_trainMean)
y_predMean = clfMean.predict(X_testMean)
print(round(metrics.accuracy_score(y_testMean, y_predMean)*100,2),"%")

clfMedian = DecisionTreeClassifier(random_state=1)
clfMedian = clfMedian.fit(X_trainMedian,y_trainMedian)
