...
import pandas as pd
#Clasificador a utilizar: Regresión Logistica
...
X_trainMean, X_testMean, y_trainMean, y_testMean = train_test_split(
             XMean, y, test_size = 0.2, random_state=7)

X_trainMedian, X_testMedian, y_trainMedian, y_testMedian = train_test_split(
             XMedian, y, test_size = 0.2, random_state=7)

X_train, X_test, y_train, y_test = train_test_split(
             X, y, test_size = 0.2, random_state=7)
 
clf = LogisticRegression(multi_class='multinomial', penalty="none" ,solver='newton-cg', max_iter=600, random_state=3)
clf.fit(X, y)
clf.predict(X)
print(round(clf.score(X, y)*100,2), "%")


clfMean = LogisticRegression(multi_class='multinomial', penalty="none" ,solver='newton-cg', max_iter=200,   random_state=7)
clfMean.fit(XMean, y)
clfMean.predict(XMean )
print(round(clfMean.score(XMean , y)*100,2), "%")

clfMedian = LogisticRegression(multi_class='multinomial',  penalty="none" ,solver='newton-cg', max_iter=200,  random_state=7)
clfMedian.fit(XMedian, y)
clfMedian.predict(XMedian)
print(round(clfMedian.score(XMedian, y)*100,2), "%")
