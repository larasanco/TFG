...
def TratamientoGeneral(u):
    ...
    for user in sesion:
        ...
        for array in Splited:
            ...
            #Usamos el método para calcular el tiempo entre cambios
            TUU = Up_Up(list(array))
            
            #Usamos el método para calcular el tiempo de entre pulsaciones
            ...
            if(len (TDU) > 10 and len (TUU) > 5 and 
               len (TDD) > 5 and len (TUD) > 5  ):
                ...
                #Guardamos los tiempos junto al usuario que lo realiza 
                    ...
                    TUUMeanTotal.append(float(round(np.mean(np.array(TUD), dtype=np.float64),10)))
                    TDDMeanTotal.append(float(round(np.mean(np.array(TDD), dtype=np.float64),10)))
                    TUDMeanTotal.append(float(round(np.mean(np.array(TUU), dtype=np.float64),10)))
                    
                    TDUMeadianTotal.append(float(round(np.median(np.array(TDU)),10)))
                    TUUMeadianTotal.append(float(round(np.median(np.array(TUD)),10)))
                    TDDMeadianTotal.append(float(round(np.median(np.array(TDD)),10)))
                    TUDMeadianTotal.append(float(round(np.median(np.array(TUU)),10)))
                    
                    TDUStdTotal.append(float(round(np.std(np.array(TDU)),10)))
                    TUUStdTotal.append(float(round(np.std(np.array(TUD)),10)))
                    TDDStdTotal.append(float(round(np.std(np.array(TDD)),10)))
                    TUDStdTotal.append(float(round(np.std(np.array(TUU)),10)))
